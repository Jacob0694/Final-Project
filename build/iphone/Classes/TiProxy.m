...
//Rare exceptions to indicate a bug in the _finalproject code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_finalproject = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_finalproject]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.finalproject.module"
